...
class FiFWebClient:
    ...
    def manual_login(self):
        ...
        link = self.page.locator("span", has_text="FiF口语训练系统")
        if link.count() > 0:
            self._log_msg("[FiF] 正在通过入口进入口语训练系统...")
            try:
                ...
                self.user_auth["token"] = page1.evaluate("localStorage.getItem('Authorization')")
                ...
                self._log_msg(f"[FiF] 弹窗失败: {e}，尝试当前页面...")
                self.page.wait_for_timeout(3000)
                self.user_auth["token"] = self.page.evaluate("localStorage.getItem('Authorization')")
                self.user_auth["source"] = self.page.evaluate("localStorage.getItem('source')")
        else:
    ...
    def get_user_info(self):
        ...
        for k in ["userId", "user_id", "uid", "id"]:
            if k in ls_data and ls_data[k]:
                userId = str(ls_data[k])
                break
        for k in ["realName", "userName", "nickname", "name"]:
            if k in ls_data and ls_data[k]:
                realName = str(ls_data[k])
                break
        if not userId and token and token.count(".") == 2:
            import base64
            try:
                payload = token.split(".")[1]
                payload += "=" * (4 - len(payload) % 4)
                jwt = json.loads(base64.urlsafe_b64decode(payload))
                for f in ["user_id", "userId", "sub", "id"]:
                    if f in jwt and jwt[f]:
                        userId = str(jwt[f])
                        break
                for f in ["realName", "name", "userName"]:
                    if f in jwt and jwt[f]:
                        realName = str(jwt[f])
                        break
            except Exception:
                pass
        if not userId:
            raise Exception(f"无法获取 userId。ls_keys: {list(ls_data.keys())}")
        self.user_info = {"data": {"userId": userId, "realName": realName or str(userId)}}
        return self.user_info
    ...
    def reset_page(self):
        ...
        try:
            old_page = self.page
            old_page.goto("about:blank")
            self.page = self.context.new_page()
            old_page.close()
            self._log_msg("[FiF] 页面已重置")
        except Exception as e:
    ...
    def start_level_test(self, page: Page, speaker, unit_id, task_id, level_id, level_name: Optional[str] = None):
        ...
        # 检测是否为朗读模式
        # 需要合并录音的题型关键词
        _merge_keywords = ["read", "朗读", "朗诵", "passage",
            "陈述", "个人陈述", "个人", "personal", "statement",
            "talk", "speech", "presentation", "monologue", "freestyle"]
        is_read_mode = level_name and any(
            kw in (level_name or "").lower() for kw in _merge_keywords
        )
